...
class SignatureVerificationError(CyborgException):
    msg_fmt = _("Signature verification for the image "
                "failed: %(reason)s.")


class ShellExecFailed(CyborgException):
    _msg_fmt = _("Failed to execute shell command.")


class ShellReturnFailed(CyborgException):
    _msg_fmt = _("Command executed success, but returnd error.")
