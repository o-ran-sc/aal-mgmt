...
class Deployable(base.APIBase):
    ...
    """The function of the deployable"""
    ...
    programable = types.boolean
    """Whether the deployable is programable"""
...
class ProgramDeployablesController(rest.RestController):
    """REST controller for program firmware Deployables."""
    # url path: /v1/deployables/program/{uuid}

    @policy.authorize_wsgi("cyborg:deployable", "create", False)
    @expose.expose(None, types.uuid, types.uuid)
    def post(self, uuid, firmware_uuid):
        """Program a deployable with glance firmware.
        :param uuid: the uuid of a deployable.
        :param firmware_uuid: the uuid of glance firmware.
        """
        context = pecan.request.context
        obj_dep = objects.Deployable.get(context, uuid)

        if obj_dep.programable is False:
            message = 'Deployable %s is not programable' % obj_dep.uuid
            raise exception.Conflict(message=message)
        elif obj_dep.availability not in ['free', 'error']:
            message = 'Deployable %s is not allow to program in %s status' % \
                      (obj_dep.uuid, obj_dep.availability)
            raise exception.Conflict(message=message)
        else:
            pecan.request.agent_api.fpga_program(context, obj_dep, firmware_uuid)

class DeployablesController(rest.RestController):
    """REST controller for Deployables."""

    program = ProgramDeployablesController()
